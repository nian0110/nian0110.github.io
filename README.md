# nian0110.github.io
```
python ./script/copy_random_images.py all
```
```
python ./script/compress_images.py 200 ./image_process/tmp ./image_process/compress
```
```
python ./script/export_to_csv.py false
python ./script/export_to_csv.py false ./images/linglingkwong ./data/images_ling.csv
python ./script/export_to_csv.py false ./images/ormkornnaphat ./data/images_orm.csv
python ./script/export_to_csv.py false ./images/mix ./data/images_mix.csv
python ./script/export_to_csv.py false ./images/OnlyYouSeriesTH ./data/images_onlyyou.csv
python ./script/export_to_csv.py false ./images/SOU_Ch3Official ./data/images_tsou.csv
```