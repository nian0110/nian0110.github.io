...
fetch('images.csv')
    ...
    .then(data => {
        ...
        // 初始隨機選取 5 張圖片
        selectedImages = shuffleArray(imageUrls).slice(0, 5);
        count.innerHTML = "共 " + totalImagesCount + " 張圖片，已隨機選取 5 張圖片，剩餘 " + selectedImages.length + " 張圖片"; // 更新圖片數量
    })
    .catch(error => {
...
button.addEventListener("click", function() {
    if (selectedImages.length > 0) {
        ...
        count.innerHTML = "共 " + totalImagesCount + " 張圖片，剩餘 " + selectedImages.length + " 張圖片"; // 更新剩餘圖片數量
    } else {
...
refreshButton.addEventListener("click", function() {
    // 當按下 refresh 按鈕時，重新隨機選取 5 張圖片
    if (imageUrls.length >= 5) {
        selectedImages = shuffleArray(imageUrls).slice(0, 5); // 隨機選取 5 張圖片
        removedUrls = []; // 清空已顯示的圖片列表
        image.src = ""; // 清除顯示的圖片
        image.style.display = "none"; // 隱藏圖片
        filename.innerHTML = ""; // 清除檔名顯示
        count.innerHTML = "共 " + totalImagesCount + " 張圖片，已隨機選取 5 張圖片，剩餘 " + selectedImages.length + " 張圖片"; // 更新剩餘圖片數量
    } else {
        alert("圖片數量不足以隨機選取 5 張。");
    }
});
